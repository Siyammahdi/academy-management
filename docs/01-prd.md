...
| **Manager** | Assigned batch(es) only | Verifies/rejects manual payments, grants grace, views their batch's students and reporting. Cannot move money or leave their batch scope. |
...
A single person may hold **multiple roles** (e.g. a manager who is also enrolled as a student).
...
**Batch** — a concrete instance of a course, created fresh each time enrollment opens. **This is the enrollable unit.** Owns capacity, enrollment window, course start date, payment due window, assigned managers, and a **frozen snapshot of the course's fees** plus an optional entry discount.
...
4. **Gateway payment → enrolled instantly.** **Manual payment → enrollment `pending`** until a manager verifies.
...
- **Timely-pending submissions are protected.** A pending payment or pending request shields the student. If a manager later *rejects* it, the penalty applies at the point of rejection.

---

## 8. Payments

**Channels:** gateway (SSLCommerz — trusted, instant) and manual (transaction reference + proof screenshot — `pending` until a manager verifies).
...
| Approver | Manager **or** admin | **Admin only** |
...
**A manager may never decide a request, verify a payment, or grant grace on their own enrollment** — it must escalate to an admin.
...
**Policy:** student-facing events → **dashboard + email**. Manager-facing events → **dashboard only**.
...
| Pending payments awaiting verification | Manager |
| Students at risk of penalty | Manager |

---

## 12. Reporting

Scope follows the permission boundary: **managers see only their batch(es); admins see academy-wide.**
