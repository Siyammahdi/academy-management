...
| Issue a refund | — | — | ✓ |
| **Class management (added scope)** | | | |
| Set/edit a batch's class link | — | ⚠ own batches | ✓ |
| Create/edit/delete homework on a batch | — | ⚠ own batches | ✓ |
| Read homework across own active enrollments | ✓ | — | — |
| Create/edit/delete a recorded class on a batch | — | ⚠ own batches | ✓ |
| Read recordings across own active enrollments | ✓ | — | — |
| **Requests** | | | |
...
**Requests, reporting, and role-management rows describe the specified target state.** As of `10-current-state.md`, only the rows above the "Requests" section (through class management) are implemented and enforced by real guards. See `12-roadmap.md` for what remains.
...
Mechanically it is one comparison at the point of decision, applied at three call sites: payment verification, payment rejection, and request decision. Class-management actions (link, homework, recordings) are not money- or request-decisions, so `SelfApprovalGuard` does not apply to them — only `BatchScopeGuard` does.
...
`BatchScopeGuard`/`SelfApprovalGuard` resolve "which batch/student does this request target?" via a `@TargetResource(kind)` decorator and a shared `resolveTarget()` walk. `kind` is one of: `batch` (the `:id` param **is** the batch id), `enrollment`, `billingPeriod`, `payment`, `request`, `homework`, `recording`. Adding a new batch-scoped resource means adding one case to that walk — see `homework`/`recording` for the pattern (both resolve `{ batchId }` only; neither has an owning student, so `SelfApprovalGuard` is never applied to them).
...
A batch-scoped, non-money route (e.g. homework) omits `SelfApprovalGuard` and instead sets `@TargetResource('homework')` so `BatchScopeGuard` resolves the batch from the homework id, not the route's `:id` param:
...
| `POST /auth/forgot-password`, `POST /auth/reset-password` | Forgot-password always `200` (no email enumeration); rate-limited |
| `GET /courses`, `GET /courses/:id` | Active courses only |
| `GET /batches?status=enrolling`, `GET /batches/:id` | Open/any batch respectively — includes `classLink` once set |
| `POST /guest/lookup` | Returns student **name + outstanding dues only** (GST-05) |
| `POST /guest/pay/gateway`, `POST /guest/pay/manual` | Initiates a guest payment |
...
**The guest lookup deliberately exposes a student's name** to anyone supplying a valid identifier — the accepted tradeoff that lets a paying relative confirm they have the right student. It exposes nothing else. Rate limiting is a deferred hardening measure — see `11-hardening.md` H-04.
