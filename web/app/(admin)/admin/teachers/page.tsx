...
async function loadTeachers(): Promise<TeacherRow[]> {
  ...
  return users.map((user) => ({
    user,
    batches: detailed
      .filter((batch) => batch.teachers.some((m) => m.userId === user.id))
...
export default function AdminTeachersPage() {
  ...
  async function reload(): Promise<void> {
    try {
      setRows(await loadTeachers())
      setError(null)
    } catch {
      setError('Teachers could not be loaded. Try again.')
    }
  }
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      <AdminPageHeader
        eyebrow="Academy"
        title="Teachers"
        description="People who can verify payments and run classrooms on assigned batches. Open a teacher for full profile and role management, or assign them from a batch."
        actions={
          <Button
            variant="outline"
            className="min-h-11"
      ...
      {rows ? (
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
          <div className="rounded-xl bg-primary p-4 text-primary-foreground">
            <p className="text-xs text-primary-foreground/75">Teachers</p>
      ...
      {rows && filtered.length === 0 ? (
        <div className="rounded-xl bg-primary-wash px-5 py-12 text-center">
          <p className="font-heading text-base font-semibold text-foreground">
            {rows.length === 0 ? 'No teachers yet' : 'No matches'}
          </p>
          <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
            {rows.length === 0
              ? 'Users with the teacher role appear here. Role assignment is not in the API yet — grant the role in ops, then assign them on a batch.'
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:items-end">
                  <Button
                    ...
                    render={<Link href={`/admin/teachers/${user.id}`} />}
                  >
                    View details
                  </Button>
                  <Button
      ...
      <p className="text-xs text-muted-foreground">
        Assignment uses batch teacher endpoints. Role changes are available on
        each teacher&apos;s details page (and under Roles).
      </p>
    </div>
  )
}
