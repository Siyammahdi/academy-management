...
import { getTaughtBatches, listCourses } from '@/lib/api-client'
import type { BatchStatus, BatchWithSeats, Course } from '@/lib/api-client'

/**
 * Teacher — My Batches
 * Only batches this teacher is assigned to (GET /me/taught-batches).
 */
          ...
          {filtered.map((batch) => {
            ...
            return (
              <BatchCard
                ...
                workspaceHref={`/teacher/batches/${batch.id}`}
                facts={[batch.classLink ? 'Class link set' : 'No class link']}
                secondaryActions={[
                  {
                    label: 'Roster',
                    href: `/teacher/batches/${batch.id}/roster`,
                  },
                  {
                    label: 'Classroom',
                    href: `/teacher/batches/${batch.id}/classroom`,
                  },
                ]}
              />
            )
