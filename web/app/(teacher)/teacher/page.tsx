...
import {
  TeacherFocusCard,
  resolveTeacherFocus,
} from '@/components/teacher/teacher-focus-card'
import { TeacherBatchShelf } from '@/components/teacher/teacher-batch-shelf'
import { TeacherHomeSkeleton } from '@/components/teacher/teacher-home-skeleton'
...
async function fetchHomeData(): Promise<TeacherHomeData> {
...
export default function TeacherOverviewPage() {
  ...
  async function reload(): Promise<void> {
    try {
      ...
      setError('The teacher portal could not be loaded. Try again.')
    }
  }
  ...
  const focus = resolveTeacherFocus({
    pending,
    pendingTotal,
    ...
    <div className="flex min-w-0 flex-col gap-5 sm:gap-7">
        ...
        actions={
          <div className="grid min-w-0 grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            <Button
              ...
              render={<Link href="/teacher/payments" />}
            >
              <ClipboardCheckIcon />
              Verify
            </Button>
            <Button
              variant="secondary"
              className="min-h-11 min-w-0"
              render={<Link href="/teacher/batches" />}
            >
              <UsersIcon />
              Your batches
            </Button>
          ...
          {homeworkDueToday.length === 0 ? (
            ...
            <ul className="flex flex-col gap-2">
              {homeworkDueToday.slice(0, 5).map((hw) => (
                ...
                >
                  <Link
                    href={`/teacher/batches/${hw.batchId}/classroom`}
        ...
        <section className="min-w-0 space-y-3">
          <div className="flex items-end justify-between gap-2">
            ...
            <Button
              variant="ghost"
              className="min-h-11 shrink-0"
              render={<Link href="/teacher/recordings" />}
            >
              View all
            </Button>
          </div>
            ...
            <ul className="flex flex-col gap-2">
              {recentRecordings.map((rec) => (
                <li key={rec.id} className="rounded-xl bg-muted/50 px-4 py-3">
                  <Link
                    href={`/teacher/batches/${rec.batchId}/classroom`}
                    className="block min-w-0 space-y-0.5"
                  >
                    <p className="truncate font-medium text-foreground">
                      {rec.title}
      ...
      {atRiskCount > 0 ? (
        <div className="flex flex-col gap-3 rounded-xl bg-status-overdue-bg p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
          ...
          <Button
            className="min-h-11 shrink-0"
            render={<Link href="/teacher/students?penalty=1" />}
          >
            Review students
          </Button>
        </div>
